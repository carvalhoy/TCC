...
# def objetivo(parameters):
#     global params, dados_S, dados_P
#     t_step = [0, 240]
#     t_solve_ivp = np.linspace(0, 240, 25)
#     x = [42500.0, 25200.0, 0.0]
   

#     sol = scipy.integrate.solve_ivp(model, t_step, x, 'DOP853', t_eval=t_solve_ivp, args=parameters, max_step= 0.05)
#     sol_S, sol_X, sol_P = sol.y
#     n = len(sol_S)
#     erro_S = (dados_S['concentração'] - sol_S)**2
#     erro_P = (dados_P['concentração'] - sol_P)**2
#     sum_erro = sum(erro_S) + sum(erro_P)
    
   
#     MSE = (1/n)*sum_erro
#     return MSE
# dados_P = pd.read_csv(r"C:\Users\claro\OneDrive\Documentos\UTFPR\TCC\Código\Rao et al. produto multiplicado.csv",";", header=None, names=["tempo", "concentração"], decimal=",")
# dados_S = pd.read_csv(r"C:\Users\claro\OneDrive\Documentos\UTFPR\TCC\Código\Rao et al. substrato multiplicado.csv",";", header=None, names=["tempo", "concentração"], decimal=",")
...
#rotina odeint
t = dados_S['tempo']
odeint = scipy.integrate.odeint(model, x, t, args=tuple([params]), tfirst=True)
S_sol = odeint[:,0]
X_sol = odeint[:,1]
P_sol = odeint[:,2]
