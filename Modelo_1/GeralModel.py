...
def modelP (t, x, params:tuple):
    ...
    S_in =     params[0] #kg_DQO_S/m3
    mu_max_X = params[1] #dia^-1
    K_S =      params[2] #kg_DQO_S/m^3
    Y_X_S =    params[3] #kg_DQO_X/kg_DQO_S
    Y_P_S =    params[4] #kg_DQO_P/kg_DQO_S
...
def subplts(data, parametros_S, parametros_P, parametros_S_P, metodoIntegracao , t_step, t_solve_ivp, x, rtol, atol, metodoOtimizacao):
    ...
    fit_S = integracao(metodoIntegracao, t_step, parametros_S, None, x, rtol, atol)
    # fit_S = scipy.integrate.solve_ivp(modelP, t_step, x, metodoIntegracao, args=[parametros_S], max_step=0.01)

    fit_P = integracao(metodoIntegracao, t_step, parametros_P, None, x, rtol, atol)
    # fit_P = scipy.integrate.solve_ivp(modelP, t_step, x, metodoIntegracao, args=[parametros_P], max_step=0.01)
    fit_S_P = integracao(metodoIntegracao, t_step, parametros_S_P, None, x, rtol, atol)
